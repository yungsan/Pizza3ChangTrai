...
<div class="flex flex-col overflow-y-auto md:flex-row mb-8">
    <div class="h-32 md:h-auto md:w-1/2 cursor-pointer hover:opacity-50" id="thumbnail">
        <img aria-hidden="true" class="object-cover w-full h-full dark:hidden thumb" src="pages/categories<?php echo $result['thumbnail'] ?>"
            alt="Office" />
    ...
    <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
        <div class="w-full">
            ...
            <input type="text" id='cid' value="<?php echo $id ?>" hidden>
            <label class="block text-sm">
                <span class="text-gray-700 dark:text-gray-400">Tên chuyên mục</span>
                <input
                    class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                    placeholder="Xuc xích Đức" id="category_name" value="<?php echo $result['category_name'] ?>" />
            </label>
...
<script>
    ...
    function sendData() {
        ...
        $.ajax({
            url: "pages/categories/handleUpdate.php",
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
